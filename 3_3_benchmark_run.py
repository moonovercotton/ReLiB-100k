import os
os.environ["CUDA_VISIBLE_DEVICES"] = "3, 5, 6"   # 只让程序看到物理 GPU0
...
# 配置
# =========================
# model_names = ["MLP", "LSTM", "TCN", "CNN", "iTransformer", "DLinear", "PatchTST", "SparseTSF",
#                "TimeMixer"]  # 可扩展
model_names = ["TimesNet"]
# capacities = [15.5, 20, 23, 30, 52, 63, 67, 105]
capacities = [30, 67, 105]
# capacities = [52, 63, 67, 105]
# capacities = [15.5]

# 3_3_benchmark.py 也要同步修改
random_seed = 621
...
def evaluate_params(gpu_id, capacity, batch_size, lr, dropout, embedding_dim, model_name):
    ...
    if result.returncode != 0 or "RuntimeError" in result.stderr or "CUDA out of memory" in result.stderr:
        ...
        error_file = f"./outputs/{model_name}_{cap_clean}_error_{random_seed}.txt"
...
def train_capacity_on_gpu(gpu_id, capacity, best_params, model_name):
    ...
    output_file = f"./outputs/{model_name}_benchmark_{cap_clean}_{random_seed}.txt"
