...
# ================== 单个容量训练函数 ==================
def run_on_gpu(gpu_id, cap, train_type, encoder_name):
    # os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    device = torch.device(f"cuda:{gpu_id}" if torch.cuda.is_available() else "cpu")

    output_dir = "./outputs"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"Ours_pretrain_{encoder_name}_{cap}_{random_seed}.txt")
    sys.stdout = open(output_path, "w", buffering=1)
    
    if train_type == 'single':
        data_file = f"./data/generated_data_full_{random_seed}/{cap}_train.pkl" 
        model_save_path = f"./cache/saved_models/{cap}_contrastive_{encoder_name}_{random_seed}.pth" 
        loss_plot_path = f'./plot/loss_plot/{encoder_name}_{cap}Ah_contrastive_loss_{random_seed}.png' 
    elif train_type == 'mix': 
        ...
        loss_plot_path = f'./plot/loss_plot/{encoder_name}_multi_without_{cap}Ah_contrastive_loss.png'
    ...
    contrastive_loss = ContinuousSupConLoss(temperature=0.2, sigma=0.05)
...
# encoder_name = "TCNEncoder"
# encoder_name = "RNNEncoder"
...
def main():
    ...
    available_gpus = [2, 3, 4, 5]  # 只使用部分 GPU
    ...
    for i in range(0, len(CAPACITY_LIST), batch_size):
        ...
        for gpu_id, cap in zip(available_gpus, batch):
            p = mp.Process(target=run_on_gpu, args=(gpu_id, cap, train_type, encoder_name))
            p.start()
            processes.append(p)
