...
def train_regression(encoder, regressor, dataloader, criterion, optimizer, device):
    ...
    for features, labels, _ in dataloader:
        ...
        # w/o CNNEncoder    main 函数里的 regressor 要同步修改
...
def main(capacity=15.5,
    ...
    # 3_2_finetune_run.py 要同步修改
    random_seed = 42    # 42 418 621
    encoder_name = 'CNNEncoder'     # CNNEncoder TCNEncoder RNNEncoder LinearEncoder
    
    if flag == 'benchmark':
        model_save_path = f'./cache/saved_models/{capacity}_contrastive_{encoder_name}_{random_seed}.pth'
        train_set = BatteryDataset(f"./data/generated_data_full_{random_seed}/{capacity}_train.pkl", flag='cap_regression')
        valid_set = BatteryDataset(f"./data/generated_data_full_{random_seed}/{capacity}_valid.pkl", flag='cap_regression')
        test_set = BatteryDataset(f"./data/generated_data_full_{random_seed}/{capacity}_test.pkl", flag='cap_regression')
        loss_path = f"./plot/loss_plot/Ours_{encoder_name}_benchmark_{capacity}_{random_seed}.png"
    elif flag == 'zero_shot':
        model_save_path = f'./cache/saved_models/multi_without_{capacity}_contrastive_{encoder_name}_{random_seed}.pth'
        train_set = BatteryDataset(f"./data/generated_data_sampling_3000/multi_without_{capacity}_train", flag='soh_regression')
        valid_set = BatteryDataset(f"./data/generated_data_sampling_3000/multi_without_{capacity}_valid", flag='soh_regression')
        test_set = BatteryDataset(f"./data/generated_data_sampling_3000/{capacity}_test", flag='soh_regression')
        loss_path = f"./plot/loss_plot/Ours_zero_shot_{capacity}_{random_seed}.png"
    ...
    # w/o CNNEncoder    train_regression 和 valid_regression 里要同步修改
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 没有命令行参数时，使用默认调用
        main(capacity=15.5, flag='benchmark', epochs_regression=300, enc_out_dim=64)