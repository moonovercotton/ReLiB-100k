import os
os.environ["CUDA_VISIBLE_DEVICES"] = "2, 3, 4, 5"   # 只让程序看到物理 GPU0
...
# capacities = [15.5, 20, 23, 30]
# capacities = [52, 63, 67, 105]
# capacities = [1, 15.5]

# 3_2_finetune.py 要同步修改
random_seed = 42
...
def evaluate_params(capacity, enc_out_dim, flag, batch_size, lr, dropout, embedding_dim, model_name):
    ...
    try:
        ...
        print(f"[Error] {model_name} {encoder_name} cap={capacity} batch_size={batch_size} 执行失败: {e}")
        return np.inf
    ...
    if result.returncode != 0 or "RuntimeError" in result.stderr or "CUDA out of memory" in result.stderr:
        ...
        error_file = f"./outputs/{model_name}_{encoder_name}_{cap_clean}_error_{random_seed}.txt"
        with open(error_file, "w") as f:
            f.write(f"超参数: batch_size={batch_size}, lr={lr}, dropout={dropout}, emb={embedding_dim}\n\n")
            f.write("===== 错误信息 =====\n")
            f.write(err_msg)
        print(f"[Error] {model_name} {encoder_name} cap={capacity} batch_size={batch_size} 出错，日志已写入 {error_file}")
        return np.inf
...
def train_capacity_on_gpu(gpu_id, capacity, enc_out_dim, best_params, flag, model_name):
    ...
    output_file = f"./outputs/{model_name}_{flag}_{encoder_name}_{cap_clean}_{random_seed}.txt"
    ...
    if result.returncode != 0 or "CUDA out of memory" in result.stderr:
        error_file = f"./outputs/{model_name}_{encoder_name}_{cap_clean}_error.txt"
        with open(error_file, "w") as f:
            f.write(f"===== 训练失败 =====\n")
            f.write(result.stderr)
        print(f"[Error] {model_name} {encoder_name} cap={capacity} 训练失败，日志已写入 {error_file}")
    else:
        print(f"[GPU {gpu_id}] {model_name} {encoder_name} cap={capacity} 训练完成，结果已保存到 {output_file}")
...
def main():
    ...
    assert num_gpus > 0, "未检测到可用 GPU"

    batch_size = num_gpus

    for batch_start in range(0, len(capacities), batch_size):
        batch_caps = capacities[batch_start: batch_start + batch_size]

        print(f"开始训练 batch {batch_start // batch_size + 1} / "
            f"{(len(capacities) + batch_size - 1) // batch_size}，"
            f"capacities = {batch_caps}")

        processes = []

        for gpu_id, cap in enumerate(batch_caps):
            p = mp.Process(
                target=train_capacity_on_gpu,
                args=(gpu_id, cap, enc_out_dim, best_params, flag, model_name)
            )
            p.start()
            processes.append(p)
        ...
        print(f"batch {batch_start // batch_size + 1} 完成\n")
